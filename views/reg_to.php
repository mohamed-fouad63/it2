...
<div class="row">
    <div class="col-6">
        <fieldset>
            <legend>بيانات التصدير</legend>
            <div id="reg_to_form">
                <div class="input-group mb-3">
                    <label class="input-group-text" id="inputGroupSelect01">التاريخ</label>
                    <input type="date" class="form-control col-sm-6  me-3" id="date_reg_to" readonly value="<?php echo date('Y-m-d'); ?>">
                ...
                <div class="input-group d-flex flex-nowrap mb-3">
                    <label class="input-group-text col-sm-3">المرسل اليه</label>
                    <input type="text" class="form-control col-sm-9" id="name_reg_to">
                </div>
                <div class="input-group mb-3">
                    <label class="input-group-text col-sm-3">الموضوع</label>
                    <textarea type="search" class="form-control col-sm-9" id="sub_reg_to" style="height: 110px;"></textarea>

                </div>
                <div class="mx-auto w-25">
                    <button class="btn btn-primary" id="add_reg_to_btn"><i class="bi bi-plus"></i></button>
                    <button class="btn btn-success" id="edit_reg_to_btn" style="display:none" value=""><i class="bi bi-pencil-square"></i></button>
                </div>
            </div>
        </fieldset>
    </div>
        ...
        <fieldset>
            <legend>
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    <div class="d-flex">
                    <input type="text" class="input_reg_to_sub ms-2">
                        <button class="btn btn-success append_reg_to_sub ms-2"><i class="bi bi-check-lg"></i></button>
                        <button class="btn btn-danger"><i class="bi bi-x-lg"></i></button>
                        </div>
                </ul>
</span>
            </legend>
            <div class="reg_to_sub_container border border-success">
            <div class="d-flex align-content-start flex-wrap pt-1">
            <?php

                $reg_to_sub_json = '../jsons/'.$db.'/';
                if(is_dir($reg_to_sub_json)){
                    if(is_file($reg_to_sub_json.'reg_to_sub.json')){
                        $reg_to_sub = file_get_contents($reg_to_sub_json.'reg_to_sub.json');
                    } else {
                        file_put_contents($reg_to_sub_json.'reg_to_sub.json', '[]');
                        $reg_to_sub = file_get_contents($reg_to_sub_json.'reg_to_sub.json');
                    }
                ...
                $arr_to_reg_sub = json_decode($reg_to_sub, true);

                foreach ($arr_to_reg_sub as $x => $val) {
                    echo '
                    <div class="btn-group mb-1 ms-1 reg_to_sub_div" role="group">
                        <button type="button" class="btn btn-secondary reg_to_sub" onclick="add_reg_to_sub(this)">' . $val . '</button>
                        <button type="button" class="btn btn-warning reg_to_sub_del" onclick="reg_sub_del(this)"><i class="bi bi-x-lg"></i></button>
                    </div>
                    ';
}
?>
    ...
    <script src="../data_tables/reg_to.js"></script>
    <script src="../js/reg_to/add_reg_to.js"></script>
    <script src="../js/reg_to/remove_reg_to.js"></script>
    <script src="../js/reg_to/edit_reg_to.js"></script>
    <script src="../js/reg_to/sent_to_by_change.js"></script>
    <script src="../js/reg_to/append_reg_to_sub.js"></script>
    <script src="../js/reg_to/add_reg_to_sub.js"></script>
    <script src="../js/reg_to/reg_sub_del.js"></script>
    <script src="../js/reg_to/easyAutocomplete.js"></script>
    <script src="../js/log/change_password.js"></script>
    <script>
        const ps = new PerfectScrollbar('.reg_to_sub_container');
