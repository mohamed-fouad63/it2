...
function get_select_office() {
  ...
  $.getJSON("../api/office/selesct_office_name2.php", function (data1) {
    ...
    $(".select_office").html(options_select_office_name_to);
  });
}
    ...
    function (data) {
      $.each(data, function (key, val) {
        options_select_it_name +=
          "<option value='" + val.sn + "'>" + val.pos_terminal + "</option>";
      });
...
function change_v200t_terminal() {
  $("#v200t tbody tr td").on("change", ".select_v200t_terminal", function () {
    ...
    // v200t.cell(0, 3).data(v200t_terminal);
...
function get_row_data() {
  $("#v200t tbody").on("click", ".btn-success", function () {
    ...
    formData = {
      ...
      user_name: tr.find("td:eq(4)").text(),
      user_auth: tr.find("td:eq(5)").find(":selected").text(),
      user_id: tr.find("td:eq(6)").text(),
      user_auth_action: tr.find("td:eq(7)").find(":selected").text(),
    };
